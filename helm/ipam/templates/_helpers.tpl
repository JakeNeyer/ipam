...
{{- end }}
{{- $mtls := default dict $p.clientMtlsConfig }}
- name: {{ $prefix }}CLIENT_MTLS_ENABLED
  value: {{ default false $mtls.enabled | quote }}
{{- if $mtls.enabled }}
{{- if not (and $mtls.tlsCertFile $mtls.tlsKeyFile) }}
{{- fail "oauth.providers[].clientMtlsConfig.enabled=true requires both tlsCertFile and tlsKeyFile to be set" }}
{{- end }}
- name: {{ $prefix }}CLIENT_MTLS_CERT_FILE
  value: {{ $mtls.tlsCertFile | quote }}
- name: {{ $prefix }}CLIENT_MTLS_KEY_FILE
  value: {{ $mtls.tlsKeyFile | quote }}
- name: {{ $prefix }}CLIENT_MTLS_TLS_VERSION
  value: {{ default "1.2" $mtls.tlsVersion | quote }}
{{- end }}
{{- end }}
{{- end }}
{{- end }}
